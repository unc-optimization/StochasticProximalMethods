...
def Prox_SARAH(x, y, x_train, y_train, x_test, y_test, LR, LR_COMP, LBD, GAMMA, \
               grad_batch_size, inner_batch_size, MAX_TOTAL_EPOCH, MAX_INNER_ITERATION, \
    ...
    w_hat_list      = [tf.Variable(tf.zeros(list(w.get_shape())), dtype = tf.float32) for w in w_list]
    ...
    # assign operations
    for grad_prev, v, v0, w, w_hat, grad, grad_map in zip(grad_prev_list, v_list, v0_list, w_list, w_hat_list, grad_list, grad_map_list):
        ...
        ops_cal_norm_v0_sq.append(norm_v0_sq.assign_add(tf.reduce_sum(tf.multiply(v0,v0))))
        ops_cal_norm_v_sq.append(norm_v_sq.assign_add(tf.reduce_sum(tf.multiply(v,v))))
        ops_cal_norm_grad_map_sq.append(norm_grad_map_sq.assign_add(tf.reduce_sum(tf.multiply(grad_map, grad_map))))
        ops_cal_norm_l1_w.append(norm_l1_w.assign_add(tf.reduce_sum(tf.abs(w))))

        # ProxSARAH update operations
        ops_store_grad_prev.append(grad_prev.assign(grad))
        ops_sarah_update_v.append(v.assign_add(grad - grad_prev))

        # Iteration update
        ops_update_w_hat.append(w_hat.assign(prox_l1(w - lr*v, lbd*lr)))
        ops_update_w.append(w.assign((1 - gm)*w + gm*w_hat))
    ...
    trainer_cal_norm_v0_sq              = tf.group(*ops_cal_norm_v0_sq)
    trainer_cal_norm_v_sq               = tf.group(*ops_cal_norm_v_sq)
    trainer_calc_norm_grad_map_sq        = tf.group(*ops_cal_norm_grad_map_sq)
    trainer_calc_norm_l1_w               = tf.group(*ops_cal_norm_l1_w)

    # ProxSARAH update operations
    trainer_store_grad_prev     = tf.group(*ops_store_grad_prev)
    trainer_sarah_update_v      = tf.group(*ops_sarah_update_v)
    trainer_update_w            = tf.group(*ops_update_w)
    trainer_update_w_hat        = tf.group(*ops_update_w_hat)
    trainer_update_grad_map     = tf.group(*ops_update_grad_map)

    #=========================================#
    # ProxSARAH main algorithm
        ...
        while num_epoch < MAX_TOTAL_EPOCH:
            ...
            for inner in range(MAX_INNER_ITERATION):
                ...
                if inner_batch_size == 1:
                    ...
                    ## store grad prev and update w hat
                    sess.run(trainer_store_grad_prev, feed_dict={x: x_sample, y: y_sample})
                    sess.run(trainer_update_w_hat, feed_dict={lr: LR, lbd: LBD})

                    ## update w and v
                    sess.run(trainer_update_w, feed_dict={gm: GAMMA})
                    sess.run(trainer_sarah_update_v, feed_dict={x: x_sample, y: y_sample})
                    ...
                    ## update w and v
                    sess.run(trainer_update_w, feed_dict={gm: GAMMA})
                    sess.run(trainer_sarah_update_v, feed_dict={x: batch_x, y: batch_y})
